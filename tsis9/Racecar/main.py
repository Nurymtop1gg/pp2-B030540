...
car_Enemy = pygame.image.load('Racecar/redcar.png')
car_Own = pygame.image.load('Racecar/greencar.png')
bg = pygame.image.load("Racecar/AnimatedStreet.png")

# fonts
font = pygame.font.SysFont("Verdana", 60)
font_small = pygame.font.SysFont("Verdana", 20)
game_over = font.render("Game Over", True, col_BLACK)
speed = pygame.font.SysFont("Verdana", 20)


class coin_generation(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.img = pygame.transform.scale(pygame.image.load('Racecar/coin.png'), (40, 40))
...
class ownMovement(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.car = pygame.transform.scale(pygame.image.load('Racecar/blucar.png'), (110, 110))
...
class enemyMovement(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.car = pygame.transform.scale(pygame.image.load('Racecar/greencar.png'), (45, 90))
        self.rect = self.car.get_rect()
...
while 1:
    ...
    if pygame.sprite.spritecollideany(current_Own, sprite_Coin):
        ...
        coins += 1
        # increasing own_Speed
        own_Speed += 1
        time.sleep(0.1)

    # To be run if collision occurs between Player and Enemy
    if pygame.sprite.spritecollideany(current_Own, sprite_Enemy):
        pygame.mixer.Sound('Racecar/crash.mp3').play()
        time.sleep(0.7)

        display.fill(col_R)
        display.blit(game_over, (270, 300))
    ...
    spd = speed.render(f"Enemy speed  {own_Speed}", True, (0, 0, 255))
    display.blit(spd, (30, 20))
    pygame.display.update()
    fepes.tick(FPS)